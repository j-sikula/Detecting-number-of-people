...
class STMAlgorithmus {
  ...
  List<int> processMeasurement(Measurement currentFrame) {
    ...
    for (int i = 0; i < heightData.length; i++) {
      ...
      if (transposed) {
        if (i % 8 > 1 && i % 8 < 4) {
          if (rotated) {
            enterZoneSum += heightData[i];
          } else {
            exitZoneSum += heightData[i];
          }
        } else if (i % 8 > 3 && i % 8 < 6) {
          if (rotated) {
            exitZoneSum += heightData[i];
 ...
 if (currentStateValue != stateValue) {
      ...
      } else if (positionExit!=0 && currentStateValue == exitSequence[positionExit-1] && positionEnter == 0) {
        positionExit--;        
      ...
      } else if (positionEnter!=0 && currentStateValue == enterSequence[positionEnter-1]) {
        positionEnter--;
      } else {
